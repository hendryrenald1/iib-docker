...
RUN sed -i 's/localhost/servicevalsonarqube/g' /opt/sonar-scanner/conf/sonar-scanner.properties
RUN echo "sonar.projectKey=Projects" >> /opt/sonar-scanner/conf/sonar-scanner.properties 
RUN echo "sonar.projectName=IIBCodeReivewUsingSonar" >> /opt/sonar-scanner/conf/sonar-scanner.properties
RUN echo "sonar.projectVersion=1.0" >> /opt/sonar-scanner/conf/sonar-scanner.properties
 
RUN echo "sonar.modules=esql-module,messageflow-module" >> /opt/sonar-scanner/conf/sonar-scanner.properties
 
RUN echo "esql-module.sonar.language=esql" >> /opt/sonar-scanner/conf/sonar-scanner.properties
RUN echo "esql-module.sonar.projectBaseDir=/var/lib/jenkins/jobs/IIBCodeReview/workspace" >> /opt/sonar-scanner/conf/sonar-scanner.properties
RUN echo "esql-module.sonar.sources=Projects" >> /opt/sonar-scanner/conf/sonar-scanner.properties
 
RUN echo "messageflow-module.sonar.language=msgflow" >> /opt/sonar-scanner/conf/sonar-scanner.properties
RUN echo "messageflow-module.sonar.projectBaseDir=/var/lib/jenkins/jobs/IIBCodeReview/workspace" >> /opt/sonar-scanner/conf/sonar-scanner.properties
